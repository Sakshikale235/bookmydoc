...
const BookAppointment = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const { data: reviewsData, error: reviewsError } = await supabase
        .from('reviews')
        .select('id,rating,comment,created_at,patient:patients(full_name,profile_photo)')
  ...
  return (
    <div className="w-full bg-[#340238]" style={{ background: 'linear-gradient(90deg, rgb(226, 226, 226), rgb(201, 214, 255))' }}>
      <Navigation/>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <DoctorProfileCard doctor={doctor} averageRating={calculateAverageRating()} totalReviews={reviews.length}/>
        <motion.div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8">
              ...
              <React.Fragment key={s}>
                <div className="flex items-center">
                  ...
                  <span className="ml-2 text-xs sm:text-sm font-medium text-gray-600 hidden sm:inline">
                    {s===1?'Date & Time':s===2?'Confirm':'Done'}
                  </span>
                </div>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={step} initial={{ opacity:0, x:20 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-20 }}>
                  ...
                  <div className="mb-8">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex space-x-2">
                        <button onClick={()=>navigateMonth('prev')} className="p-2 hover:bg-gray-100 rounded-lg transition-colors"><ChevronLeft className="w-5 h-5"/></button>
              ...
              {step===2 && (
                <div>
                  <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center"><User className="w-6 h-6 mr-3 text-blue-500"/>Confirm Details</h2>
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <textarea name="reason" value={patientInfo.reason} onChange={handleInputChange} rows={4} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Briefly describe your symptoms or reason for consultation"/>
                    </div>
                  </div>

                  <div className="mt-8 flex justify-between">
                    <button onClick={()=>setStep(1)} disabled={submitting} className="bg-gray-200 text-gray-700 px-8 py-3 rounded-lg font-semibold hover:bg-gray-300 disabled:opacity-50 transition-colors flex items-center">
                      <ChevronLeft className="w-5 h-5 mr-2"/>Back
                    </button>
                    <button onClick={handleBookAppointment} disabled={submitting} className="bg-blue-600 text-white px-8 py-3 rounded-lg font-semibold hover:bg-blue-700 disabled:opacity-50 transition-colors flex items-center">
                      {submitting ? <><Loader2 className="w-5 h-5 mr-2 animate-spin"/>Booking...</> : <>Confirm Booking<CheckCircle className="w-5 h-5 ml-2"/></>}
                    </button>
                  </div>
                </div>
              )}
              ...
              {step===3 && (
                <div className="text-center py-8">
                  <motion.div initial={{scale:0}} animate={{scale:1}} transition={{type:'spring', duration:0.5}} className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-6">
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 justify-center">
                    <Link to="/index" className="bg-blue-600 text-white px-8 py-3 rounded-lg font-semibold hover:bg-blue-700 transition-colors">Back to Home</Link>
                    <Link to="/user_profile" className="bg-gray-200 text-gray-700 px-8 py-3 rounded-lg font-semibold hover:bg-gray-300 transition-colors">View My Profile</Link>
                  </div>
                </div>
              )}
            </motion.div>
        ...
        <ReviewsSection reviews={reviews} doctorId={doctorId!} patientId={patient?.id} onReviewSubmitted={fetchData}/>
      </div>
      <Footer/>
    </div>
  );
